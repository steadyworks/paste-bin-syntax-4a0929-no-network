...
def create_paste():
    ...
    conn = get_db()
    conn.execute(
        'INSERT INTO pastes (id, code, language, expiry_type, expiry_time, expiry_views, view_count, password_hash, created_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?)',
...
def get_paste(paste_id):
    ...
    conn = get_db()
    paste = conn.execute('SELECT * FROM pastes WHERE id = ?', (paste_id,)).fetchall()
